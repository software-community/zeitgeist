...
def tech_events(request):
    ...
    clubs = Clubs.objects.filter(category='TECH')
    return render(request, 'main_page/technical_events.html', {'clubs': clubs})


def cult_events(request):
    clubs = Clubs.objects.filter(category='CULT')
    return render(request, 'main_page/cultural_events.html', {'clubs': clubs})
...
def update_reg_database(details, email):
    ...
    for registration in Registrations.objects.all():
        if registration.email == email:
            ...
            registration.events = json.dumps(
                details['events'], cls=DjangoJSONEncoder)
